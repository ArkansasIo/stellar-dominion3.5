...
import type { EquipmentSlot, StatRarity, EquipmentItem, SubStat } from './equipmentTemperingSystem';
...
export function getDefaultSmithyState(): SmithyState {
  ...
  return {
    ...
    totalCrafted: 0, totalTempered: 0, totalMasterworked: 0, totalSalvaged: 0,
...
export function processTemper(state: SmithyState, equipment: EquipmentItem, selectedSubStatIndex: number): { success: boolean; newState: SmithyState; newEquipment?: EquipmentItem; message: string } {
  const cost = calculateTemperCost(equipment.rarity, equipment.temperCount);
  ...
  const newSubStats = [...equipment.subStats];
  if (selectedSubStatIndex >= 0 && selectedSubStatIndex < newSubStats.length) {
    const pool = [
      { statType: 'weaponDamage', name: 'Weapon Damage', min: 3, max: 40, isPercent: true },
      { statType: 'hullHp', name: 'Hull HP', min: 3, max: 30, isPercent: true },
      { statType: 'shieldHp', name: 'Shield HP', min: 3, max: 35, isPercent: true },
    ...
    const value = pick.min + Math.random() * (pick.max - pick.min);
    newSubStats[selectedSubStatIndex] = { ...newSubStats[selectedSubStatIndex], statType: pick.statType, name: pick.name, value: Math.round(value * 10) / 10, isPercent: pick.isPercent };
  }
  const newEquipment = { ...equipment, subStats: newSubStats, temperCount: equipment.temperCount + 1 };
  newState.totalTempered += 1;
  newState.experience += 10;
  newState.temperHistory = [...state.temperHistory, { equipmentId: equipment.id, timestamp: Date.now(), result: 'tempered' }];
...
export function processMasterwork(state: SmithyState, equipment: EquipmentItem): { success: boolean; newState: SmithyState; newEquipment?: EquipmentItem; message: string } {
  if (equipment.masterworkTier >= 10) return { success: false, newState: state, message: 'Maximum masterwork tier reached' };
  ...
  const success = Math.random() < (0.9 - equipment.masterworkTier * 0.05);
  if (success) {
    const newEquipment = { ...equipment, masterworkTier: equipment.masterworkTier + 1, isMasterworked: true };
    newState.totalMasterworked += 1;
    newState.experience += 50;
    newState.masterworkHistory = [...state.masterworkHistory, { equipmentId: equipment.id, timestamp: Date.now(), tier: newEquipment.masterworkTier }];
    return { success: true, newState, newEquipment, message: `Masterwork tier increased to +${newEquipment.masterworkTier}` };
  }
  newState.experience += 20;
  return { success: false, newState, message: 'Masterwork attempt failed' };
}
