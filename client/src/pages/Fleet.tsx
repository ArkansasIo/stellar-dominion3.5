  ...
  return (
    <GameLayout>
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
        <section className="overflow-hidden rounded-2xl border border-slate-200 shadow-sm bg-cover bg-center" style={{ backgroundImage: `linear-gradient(rgba(15,23,42,0.78), rgba(15,23,42,0.92)), url(${BACKGROUND_ASSETS.FLEET.path})` }}>
          <div className="p-5 lg:p-6 space-y-4 text-white">
            <div className="flex items-center gap-2">
              <img src={SHIP_ASSETS.CAPITALS.BATTLECRUISER.path} alt="Icon" className="w-8 h-8 rounded-lg border border-white/10 bg-white/10 p-1.5 object-contain" onError={(e) => { e.currentTarget.onerror = null; e.currentTarget.src = TEMP_THEME_IMAGE; }} />
              <h1 className="text-2xl font-bold">Fleet Command</h1>
            </div>
            <p className="text-sm leading-6 text-slate-300">Manage active missions and dispatch fleets across the galaxy.</p>
            <div className="flex flex-wrap gap-3">
              {[{ label: "Battlecruiser", image: SHIP_ASSETS.CAPITALS.BATTLECRUISER.path }, { label: "Fighter", image: SHIP_ASSETS.FIGHTERS.FIGHTER.path }, { label: "Interceptor", image: SHIP_ASSETS.FIGHTERS.INTERCEPTOR.path }].map((item) => (
