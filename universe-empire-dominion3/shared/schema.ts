...
export const forumThreads = pgTable("forum_threads", {
  ...
  authorName: varchar("author_name").notNull().default("Commander"),
...
export const insertForumThreadSchema = createInsertSchema(forumThreads).omit({ createdAt: true, updatedAt: true });
...
export const insertForumReplySchema = createInsertSchema(forumReplies).omit({ createdAt: true });
...
export const realmServers = pgTable("realm_servers", {
  ...
  description: text("description").notNull().default(""),
  region: varchar("region").notNull().default("global"),
  maxPlayers: integer("max_players").notNull().default(10000),
  currentPlayers: integer("current_players").notNull().default(0),
  status: varchar("status").notNull().default("active"),
  color: varchar("color").notNull().default("#6366f1"),
  bonusDescription: varchar("bonus_description").notNull().default(""),
  bonusMultiplier: real("bonus_multiplier").notNull().default(1.0),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export type RealmServer = typeof realmServers.$inferSelect;
export const insertRealmServerSchema = createInsertSchema(realmServers).omit({ createdAt: true, updatedAt: true });
export type InsertRealmServer = z.infer<typeof insertRealmServerSchema>;

// Weekly Missions - Player mission progress per week
export const weeklyMissionProgress = pgTable("weekly_mission_progress", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  weekId: varchar("week_id").notNull(),
  missions: jsonb("missions").notNull().default([]),
...
export const insertWeeklyMissionProgressSchema = createInsertSchema(weeklyMissionProgress).omit({ id: true, createdAt: true, updatedAt: true });
export type InsertWeeklyMissionProgress = z.infer<typeof insertWeeklyMissionProgressSchema>;

// Path of Ascension - Player tier progression and mission completion
export const pathOfAscension = pgTable("path_of_ascension", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  userId: varchar("user_id").notNull().references(() => users.id, { onDelete: "cascade" }),
  currentTier: integer("current_tier").notNull().default(1),
  currentTierXp: integer("current_tier_xp").notNull().default(0),
  totalXp: integer("total_xp").notNull().default(0),
  completedMissions: jsonb("completed_missions").notNull().default([]),
  claimedTiers: jsonb("claimed_tiers").notNull().default([]),
  unlockedUpToTier: integer("unlocked_up_to_tier").notNull().default(1),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});
...
// ============================================================
// Coupons — admin-created discount/promotion codes
// ============================================================
...
export const coupons = pgTable("coupons", {
  ...
  darkMatter: integer("dark_matter").notNull().default(0),
