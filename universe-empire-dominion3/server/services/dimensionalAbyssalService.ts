...
export class DimensionalAbyssalService {
  ...
  async openRaidChest(
    ...
    return {
      ...
      result: {
        chestRewards,
        remainingTokens: userContract.tokensEarned - userContract.tokensSpent - contract.maxTokensForChest,
  ...
  async getPlayerContracts(userId: string) {
    ...
    return contracts.map((c) => ({
      ...c,
      availableTokens: c.tokensEarned - c.tokensSpent,
      canOpenChest: (c.tokensEarned - c.tokensSpent) >= (getContractTier(c.contractTier)?.maxTokensForChest || 0),
    }));
  }
  ...
  async openGateChest(
    ...
    return {
      ...
      result: {
        chestRewards,
        remainingTokens: userToken.tokensEarned - userToken.tokensSpent - gate.maxTokensForChest,
  ...
  async getPlayerGates(userId: string) {
    ...
    return tokens.map((t) => ({
      ...t,
      availableTokens: t.tokensEarned - t.tokensSpent,
      canOpenChest: (t.tokensEarned - t.tokensSpent) >= (getAbyssalGateTier(t.gateTier)?.maxTokensForChest || 0),
    }));
  }
}
