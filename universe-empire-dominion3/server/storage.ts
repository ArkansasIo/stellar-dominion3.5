...
export interface IStorage {
  ...
  createWeeklyMissionProgress(data: InsertWeeklyMissionProgress): Promise<WeeklyMissionProgress>;
  updateWeeklyMissionProgress(userId: string, weekId: string, updates: Partial<WeeklyMissionProgress>): Promise<WeeklyMissionProgress>;

  // Path of Ascension operations
...
export class DatabaseStorage implements IStorage {
  ...
  async createForumReply(data: { threadId: string; authorId: string; authorName: string; content: string }): Promise<any> {
    const [reply] = await db.insert(forumReplies).values(data).returning();
    await db.update(forumThreads).set({ replyCount: sql`${forumThreads.replyCount} + 1`, lastReplyAt: new Date(), updatedAt: new Date() }).where(eq(forumThreads.id, data.threadId));
    return reply;
  }

  async deleteForumThread(threadId: string): Promise<boolean> {
    const result = await db.delete(forumThreads).where(eq(forumThreads.id, threadId));
    return (result.rowCount ?? 0) > 0;
  }

  async deleteAllForumThreads(): Promise<void> {
    await db.delete(forumReplies);
    await db.delete(forumThreads);
  }

  // Realm operations
  async getRealmServers(): Promise<any[]> {
    return db.select().from(realmServers).orderBy(realmServers.name);
  }

  async getRealmServer(slug: string): Promise<any | null> {
    const [realm] = await db.select().from(realmServers).where(eq(realmServers.slug, slug));
    return realm || null;
  }

  async createRealmServer(data: { name: string; slug: string; description: string; region: string; maxPlayers: number; color: string; bonusDescription: string; bonusMultiplier: number }): Promise<any> {
    const [realm] = await db.insert(realmServers).values(data).returning();
    return realm;
  }

  async updateRealmPlayerCount(slug: string, delta: number): Promise<void> {
    await db.update(realmServers).set({ currentPlayers: sql`GREATEST(0, ${realmServers.currentPlayers} + ${delta})`, updatedAt: new Date() }).where(eq(realmServers.slug, slug));
  }

  // Weekly Mission operations
  async getWeeklyMissionProgress(userId: string, weekId: string): Promise<WeeklyMissionProgress | undefined> {
    const [result] = await db.select().from(weeklyMissionProgress)
      .where(and(eq(weeklyMissionProgress.userId, userId), eq(weeklyMissionProgress.weekId, weekId)));
    return result;
  }

  async createWeeklyMissionProgress(data: InsertWeeklyMissionProgress): Promise<WeeklyMissionProgress> {
    const [result] = await db.insert(weeklyMissionProgress).values(data).returning();
    return result;
  }

  async updateWeeklyMissionProgress(userId: string, weekId: string, updates: Partial<WeeklyMissionProgress>): Promise<WeeklyMissionProgress> {
    const [result] = await db.update(weeklyMissionProgress)
      .set({ ...updates, updatedAt: new Date() })
      .where(and(eq(weeklyMissionProgress.userId, userId), eq(weeklyMissionProgress.weekId, weekId)))
      .returning();
    return result;
  ...
  // Path of Ascension operations
  async getAscensionProgress(userId: string): Promise<PathOfAscension | undefined> {
    const [result] = await db.select().from(pathOfAscension).where(eq(pathOfAscension.userId, userId));
    return result;
  }

  async createAscensionProgress(data: InsertPathOfAscension): Promise<PathOfAscension> {
    const [result] = await db.insert(pathOfAscension).values(data).returning();
    return result;
  }

  async updateAscensionProgress(userId: string, updates: Partial<PathOfAscension>): Promise<PathOfAscension> {
    const [result] = await db.update(pathOfAscension)
      .set({ ...updates, updatedAt: new Date() })
      .where(eq(pathOfAscension.userId, userId))
      .returning();
    return result;
  }
}
