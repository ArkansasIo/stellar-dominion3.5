  ...
  app.post("/api/settings/player/export", isAuthenticated, async (req: Request, res: any) => {
    try {
      ...
      const exportData = {
        ...
        notifications: (setting?.value as any)?.notifications || DEFAULT_PLAYER_OPTIONS.notifications,
        display: (setting?.value as any)?.display || DEFAULT_PLAYER_OPTIONS.display,
        sound: (setting?.value as any)?.sound || DEFAULT_PLAYER_OPTIONS.sound,
        privacy: (setting?.value as any)?.privacy || DEFAULT_PLAYER_OPTIONS.privacy,
